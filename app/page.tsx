  ...
  return (
    <main className="welcome-shell">
      ...
      <section className="welcome-hero" aria-labelledby="welcome-title">
        <Image className="welcome-hero-background" src="/playwell-portfolio-hero.png" alt="로봇과 코딩으로 포트폴리오를 만드는 플레이웰 학습 장면" fill priority sizes="100vw" unoptimized />
        <div className="welcome-hero-shade" aria-hidden="true" />
        <div className="welcome-copy">
          <span className="welcome-kicker"><i /> PLAYWELL ROBOT PORTFOLIO LAB</span>
          <h2 id="welcome-title">
            <span>로봇·코딩의 경험을</span>
            <em>하나의 포트폴리오</em>
          </h2>
        ...
        <ol className="workflow-cards">
          {CREATION_WORKFLOW.map((item, index) => (
            <li className={`workflow-card ${item.color}`} key={item.title}>
              ...
              <div className="workflow-tool-image">
                <Image src={item.image} alt={`${item.iconLabel} 도구 이미지`} width={112} height={112} unoptimized />
              </div>
              <h3>{item.title}</h3>
...
function SectionOrderEditor({ values, options, other, onToggle, onOther, onReorder, onPreviewFocus }: {
  ...
  const moveSection = (from: number, to: number) => {
    const next = reorderSections(values, from, to);
    ...
    <div className="input-block wide section-order-editor" onFocusCapture={onPreviewFocus}>
      ...
      <ol className="section-order-list" aria-label="선택한 포트폴리오 영역 순서">
        {values.map((section, index) => (
          <li
            ...
            onPointerMove={(event) => {
              if (!dragging) return;
              setDragPreview((previous) => previous ? {
                ...previous,
                left: event.clientX - previous.grabX,
                top: event.clientY - previous.grabY,
              } : previous);
              const target = sectionAtPoint(event);
      ...
      {dragPreview && (
        ...
        >
          ...
          <i>{String(values.indexOf(dragPreview.section) + 1).padStart(2, "0")}</i>
          <b>{dragPreview.section}</b>
          {dragPreview.section === "Competition Journey" && <small>필수</small>}
          <em>이동 중</em>
        </div>
      )}
...
export default function Home() {
  ...
  const showAppView = (view: AppView) => {
    setAppView(view);
  ...
  const showMobilePane = (pane: MobilePane) => {
    ...
    window.requestAnimationFrame(() => {
      const switcher = document.querySelector<HTMLElement>(".mobile-pane-switcher");
      const topbar = document.querySelector<HTMLElement>(".topbar");
      if (!switcher || !topbar) return;
      const top = window.scrollY + switcher.getBoundingClientRect().top - topbar.getBoundingClientRect().height - 8;
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        if (saved) {
          ...
          const savedAwards = parsed.awards?.length ? parsed.awards : INITIAL_DATA.awards;
          const hasSavedAward = savedAwards.some((award) => award.competition.trim() || award.result.trim());
          const savedSections = normalizeSections(parsed.sections);
          const restoredSections = hasSavedAward && !savedSections.includes("Certifications & Awards")
            ? [...savedSections, "Certifications & Awards"]
            : savedSections;
          const restoredMoods = parsed.moods?.length
            ? [resolveMoodPreset(parsed.moods, parsed.moodOther)]
            : [];
          setData({
            ...INITIAL_DATA,
            ...parsed,
  ...
  const updateAward = (id: string, key: string, value: string) => setData((previous) => {
    const awards = previous.awards.map((item) => item.id === id ? { ...item, [key]: value } : item);
    const sections = value.trim() && !previous.sections.includes("Certifications & Awards")
      ? [...previous.sections, "Certifications & Awards"]
      : previous.sections;
    return { ...previous, awards, sections };
  });
  ...
  if (appView === "landing") return <LandingScreen completedCount={completedCount} onShowWorkflow={() => showAppView("workflow")} />;
  if (appView === "workflow") return <WorkflowScreen onBack={() => showAppView("landing")} onEnterBuilder={() => showAppView("builder")} />;
    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="header-actions">
          ...
          <button type="button" className="workflow-button" onClick={() => showAppView("workflow")}>작업 흐름</button>
      ...
      <section className={`split-workspace mobile-show-${mobilePane}`} aria-label="포트폴리오 프롬프트 작성 화면">
        <nav className="mobile-pane-switcher" aria-label="모바일 작업 화면 전환">
          <button
            type="button"
            className={mobilePane === "form" ? "active" : ""}
            aria-pressed={mobilePane === "form"}
            onClick={() => showMobilePane("form")}
          >
            <span>01</span>
            <b>내용 입력</b>
            <small>현재 단계 작성</small>
          </button>
          <button
            type="button"
            className={mobilePane === "preview" ? "active" : ""}
            aria-pressed={mobilePane === "preview"}
            onClick={() => showMobilePane("preview")}
          >
            <span>02</span>
            <b>실시간 미리보기</b>
            <small>누적 화면 확인</small>
          </button>
        </nav>
        <aside className={`input-pane ${step.color}`} aria-live="polite">
          <header className="input-pane-header">
            <div className="step-title-row"><span className="panel-icon"><Image src={step.icon} alt={`${step.title} 단계 이미지`} width={72} height={72} unoptimized /></span><div><p>{step.phase} · STEP {String(step.number).padStart(2, "0")}</p><h2>{step.title}</h2><span>{step.summary}</span></div></div>
          ...
          <nav className="step-switcher" aria-label="작성 단계 바로가기">
            {STEPS.map((item, index) => {
              ...
              return <button key={item.number} type="button" className={`${index === current ? "active" : ""} ${complete ? "done" : ""}`} onClick={() => selectStep(index)} aria-current={index === current ? "step" : undefined} aria-label={`${item.number}단계 ${item.title}로 이동`}><span className="step-thumbnail"><Image src={item.icon} alt="" width={34} height={34} unoptimized /></span><small>{item.phase}</small><i>{complete ? "완료" : item.number}</i></button>;
            })}
          </nav>
